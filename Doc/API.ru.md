...
Модули класса **CelestialScanner** позволяют обнаруживать нерукотворные небесные объекты, такие как астероиды и планеты. Имеет следующие параметры:
  ...
  - **удельное время обработки сигнала** - .

Сканер не работает мгновенно - на обработку запроса ему требуется определённое время. Можно разбить работу сканера на несколько этапов:
...
3. **удельное время обработки сигнала** - время, требуемое сканеру на обработку сигнала при 
...
**Время обработки сигнала** прямо-пропорционально радиусу сканирования и обратно-пропорционально  минимальному радиусу искомых объектов.

Таким образом, формула для вычисления времени сканирования имеет вид:
```
t = 0.1 + 2 * R / c + processing_time * (R / min_obj_radius) 
```
, где:
  - **R** - радиус сканирования;
  - **processing_time** - удельное время обработки результатов сканирования;
  - **min_obj_radius** - минимальный радиус объектов, которые будут обнаруживаться сканером.
