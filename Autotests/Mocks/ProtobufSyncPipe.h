...
{
  using MessagesQueue = std::queue<spex::ICommutator>;
...
public:
  ...
  bool waitAny(uint32_t nSessionId, spex::ICommutator &out, uint16_t nTimeoutMs = 500);
  // Waiting for any message, that creates new session
  bool waitAny(uint32_t* pNewSessionId, spex::ICommutator &out,
...
protected:
  void storeMessage(uint32_t nSessionId, spex::ICommutator&& message) const;
